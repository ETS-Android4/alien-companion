 ...
 */
public class PostViewHolderOld extends RecyclerView.ViewHolder {
    ...
    private int upvoteResource, downvoteResource, saveResource, hideResource, moreResource, commentsResource, viewUserResource, openBrowserResource;

    private int classicScoreColor;
    ...
    public PostViewHolderOld(View itemView, PostViewType type) {
        super(itemView);
        this.viewType = type;
    ...
    private void setClassicScoreColor() {
        switch (MyApplication.currentBaseTheme) {
            case MyApplication.LIGHT_THEME:
                classicScoreColor = Color.parseColor("#757575");
                break;
            case MyApplication.DARK_THEME_LOW_CONTRAST:
                classicScoreColor = Color.parseColor("#c6c6c6");
                break;
            default:
                classicScoreColor = Color.WHITE;
                break;
        }
    }
    ...
    public void bindModel(Context context, final Submission post) {
        ...
        //Thumbnail postThumbnail = post.getThumbnailObject();
        //TODO: clean this
        if(viewType == PostViewType.smallCards || viewType == PostViewType.classic) {
        ...
        // check whether post is clicked or not
        ...
        switch (viewType) {
            ...
            case imageBoard:
                layoutPostOptions.setBackgroundColor(context.getResources().getColor(android.R.color.transparent));
                bindPostImageBoard(context, post);
                break;
            case cardDetails:
    ...
    private void bindPostClassic(Context context, Submission post) {
        scoreText.setText(post.getScore().toString()); // TODO: 1/22/2017 shorten score text
        ...
        setClassicListIconsAndTextColors(context, post, postDets1, scoreText);
    ...
    private void bindPostImageBoard(Context context, Submission post) {
        // TODO: 1/21/2017
    }
    ...
    private void setClassicListIconsAndTextColors(Context context, Submission post, TextView flairTextView, TextView scoreTextView) {
        // set flair color if flair is avaiable
        if(post.getLinkFlairText() != null) {
            String text = flairTextView.getText().toString();
            ...
            flairTextView.setText(spannableString);
        }
        ...
        if(MyApplication.currentUser != null) {
            ...
            if(post.getLikes().equals("true")) {
                scoreSpannable.setSpan(new TextAppearanceSpan(context, R.style.upvotedStyle), 0, scoreString.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
                upvoteClassic.setImageResource(R.mipmap.ic_upvote_classic_orange_48dp);
                downvoteClassic.setImageResource(downvoteResource);
            }
            else if(post.getLikes().equals("false")) {
                scoreSpannable.setSpan(new TextAppearanceSpan(context, R.style.downvotedStyle), 0, scoreString.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
                upvoteClassic.setImageResource(upvoteResource);
                downvoteClassic.setImageResource(R.mipmap.ic_downvote_classic_blue_48dp);
            }
            else {
                scoreTextView.setTextColor(classicScoreColor);
                upvoteClassic.setImageResource(upvoteResource);
                downvoteClassic.setImageResource(downvoteResource);
            }
    ...
    public void setClassicViewClickListeners(PostItemOptionsListener listener) {
        upvoteClassic.setOnClickListener(listener);
        downvoteClassic.setOnClickListener(listener);
    }
